...
`/v1/query_logs` is an authorized operational route. Records include the requesting principal and tenant, request/query identity or approved redaction, snapshot, start and end epochs, human-readable duration, separate requested and allocated CPU/RAM, cgroup-measured CPU time and peak RSS, participating Kubernetes identities, GPU fields, autoscaling evidence, and an explicit measurement scope. Configured requests or limits are never described as measured consumption. Query-log access must remain tenant-scoped, rate-limited, audit-linked, and unavailable to ordinary users without the explicit observation permission.
