...
repository, including the hidden `.github` folder.

## Launch on GitHub Pages now

### Option A — deploy from the `main` branch

Use this mode when custom GitHub Actions are unavailable or disabled:

1. Open **Settings → Pages** in `aktiver/c8`.
2. Under **Build and deployment → Source**, select **Deploy from a branch**.
3. Select branch **main**, folder **/(root)**, then click **Save**.
4. GitHub runs its built-in Jekyll build and publishes the site.

### Option B — GitHub Actions

1. Open **Settings → Pages** in `aktiver/c8`.
2. Under **Build and deployment → Source**, select **GitHub Actions**.
3. Open **Actions** and run **Deploy c8 Jekyll site to GitHub Pages**, or push one small change to `main`.
...
Then select either deployment mode above in **Settings → Pages**.
