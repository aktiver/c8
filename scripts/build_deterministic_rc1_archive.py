#!/usr/bin/env python3
"""Create a sorted, metadata-normalized, secret-screened RC1 source ZIP."""
...
def main() -> int:
    ...
    parser.add_argument("--root-name", default="NGKG_1_0_0_RC1")
    ...
    for path in sorted(source.rglob("*")):
        ...
        if any(part in FORBIDDEN_PARTS for part in relative.parts): continue
        if path.is_symlink(): raise ValueError(f"symlinks are forbidden in RC1 archives: {relative}")
        if path.is_file():
    ...
    with zipfile.ZipFile(args.output, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9, strict_timestamps=True) as archive:
        for path in paths:
            ...
            info = zipfile.ZipInfo(f"{args.root_name}/{relative}", date_time=date)
    ...
    print(json.dumps({"archive": str(args.output), "files": len(paths), "sha256": digest}, sort_keys=True))
    return 0
