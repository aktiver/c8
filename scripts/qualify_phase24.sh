...
jq -e \
  ...
  .execution.shufflePartitionCount >= 2 and .execution.shuffleWorkerCount >= 2 and
...
kubectl --namespace "${NGKG_KUBERNETES_NAMESPACE}" get statefulset ngkg-fragment-worker -o json | jq -e '
  ...
  (.spec.template.spec.containers[0].env |
    map(select(.name == "NGKG_SHUFFLE_PARTITIONS" or
               .name == "NGKG_MAX_SHUFFLE_REQUEST_BYTES" or
               .name == "NGKG_MAX_SHUFFLE_RESPONSE_BYTES" or
               .name == "NGKG_MAX_SHUFFLE_EXCHANGE_BYTES" or
               .name == "NGKG_SHUFFLE_EXCHANGE_CONCURRENCY")) | length == 5) and
  (.spec.template.spec.containers[0].env |
...
if (( ready_workers < 2 )); then
  echo "at least two ready fragment/shuffle workers are required" >&2
  exit 1
fi
...
jq -n \
  ...
  --argjson shufflePartitionCount "$(jq -r .execution.shufflePartitionCount "${response}")" \
  --argjson shuffleWorkerCount "$(jq -r .execution.shuffleWorkerCount "${response}")" \
  --arg evidenceDirectory "${run_dir}" \
  '{status: "phase24-certified-partitioned-shuffle-passed", snapshotId: $snapshotId,
    querySha256: $querySha256, planSha256: $planSha256,
    shufflePartitionCount: $shufflePartitionCount, shuffleWorkerCount: $shuffleWorkerCount,
    evidenceDirectory: $evidenceDirectory,
    qualificationBoundary: "Also run pinned Rust tests, corruption and skew matrices, Phase23 fallback equality, enterprise CPU/RSS/wire benchmarks, Helm server dry-run, sustained 79/80 percent HPA, Rancher node growth, worker-loss and missing-partition fault injection before release."}'
