...
kubectl --namespace "${NGKG_KUBERNETES_NAMESPACE}" get statefulset ngkg-fragment-worker -o json | jq -e '
  ...
  (.spec.template.spec.containers[0].env |
    map(select(.name == "NGKG_MAX_DISTRIBUTED_FRAGMENTS" or
               .name == "NGKG_MAX_DISTRIBUTED_INTERMEDIATE_ROWS" or
               .name == "NGKG_MAX_DISTRIBUTED_EXCHANGE_BYTES" or
               .name == "NGKG_MAX_FRAGMENT_RESPONSE_BYTES" or
               .name == "NGKG_MAX_RESIDENT_FRAGMENT_RUNTIMES")) | length == 5) and
  (.spec.template.spec.containers[0].env |
...
jq -n \
  ...
  --argjson workerCount "$(jq -r .execution.workerCount "${response}")" \
  --arg evidenceDirectory "${run_dir}" \
  '{status: "phase22-certified-distributed-fragments-passed", snapshotId: $snapshotId,
    querySha256: $querySha256, planSha256: $planSha256, workerCount: $workerCount,
    evidenceDirectory: $evidenceDirectory,
    qualificationBoundary: "Also run pinned Rust tests, real HermiT compilation, certificate corruption, response-limit tests, Helm server dry-run, sustained 79/80 percent HPA, Rancher node growth, worker-loss fail-closed behavior and client retry after recovery before release."}'
