...
kubectl --namespace "${NGKG_KUBERNETES_NAMESPACE}" get statefulset ngkg-query-shard -o json | jq -e '
  ...
  any(.spec.template.spec.volumes[]; .name == "shuffle-spill" and .emptyDir.sizeLimit != null) and
  (.spec.template.spec.containers[0].env |
    map(select(.name == "NGKG_SHUFFLE_SPILL_ROOT" or
               .name == "NGKG_MAX_SHUFFLE_SPILL_BYTES" or
               .name == "NGKG_MAX_SHUFFLE_OPEN_FILES")) | length == 3) and
  (.spec.template.spec.containers[0].env |
...
jq -n \
  ...
  --argjson shuffleSpillBytes "$(jq -r .execution.shuffleSpillBytes "${response}")" \
  --arg evidenceDirectory "${run_dir}" \
  '{status: "phase25-bounded-local-nvme-spill-passed", snapshotId: $snapshotId,
    querySha256: $querySha256, planSha256: $planSha256,
    shuffleSpillMode: "bounded_local_nvme_v1", shuffleSpillBytes: $shuffleSpillBytes,
    evidenceDirectory: $evidenceDirectory,
    qualificationBoundary: "Also run pinned Rust tests, full spill corruption and cancellation matrices, local-NVMe placement, scratch cleanup plateau, enterprise CPU/RSS/I/O benchmarks, Helm server dry-run, sustained 79/80 percent HPA, Rancher node growth and worker-loss injection before release."}'
