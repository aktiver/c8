...
# This gate runs a real certified, skewed cross-domain query through the
# deployed data plane. It proves exact bag equality against an independent
# expected result and proves that the coordinator streamed Arrow directly from
# checksum-validated spill files instead of constructing a complete request.
...
collect_metrics() {
  local component="$1" port="$2" destination="$3"
  shift 3
  : > "${destination}"
  local pod
  for pod in "$@"; do
...
collect_metrics query 32010 "${run_dir}/query-before.prom" "${query_pods[@]}"
collect_metrics fragment 32040 "${run_dir}/fragment-before.prom" "${fragment_pods[@]}"
...
requests_before="$(metric_sum ngkg_coordinator_streamed_shuffle_requests_total "${run_dir}/query-before.prom")"
bytes_before="$(metric_sum ngkg_coordinator_streamed_shuffle_bytes_total "${run_dir}/query-before.prom")"
...
jq -e '
  ...
  .execution.coordinatorRequestBytes > 0 and
  .execution.coordinatorRequestBytes == .execution.workerInputBytes and
...
collect_metrics query 32010 "${run_dir}/query-after.prom" "${query_pods[@]}"
collect_metrics fragment 32040 "${run_dir}/fragment-after.prom" "${fragment_pods[@]}"
requests_after="$(metric_sum ngkg_coordinator_streamed_shuffle_requests_total "${run_dir}/query-after.prom")"
bytes_after="$(metric_sum ngkg_coordinator_streamed_shuffle_bytes_total "${run_dir}/query-after.prom")"
(( requests_after > requests_before )) || {
  echo "coordinator streamed-request counter did not advance" >&2; exit 1;
}
(( bytes_after > bytes_before )) || {
  echo "coordinator streamed-byte counter did not advance" >&2; exit 1;
}
...
kubectl --namespace "${namespace}" get statefulset ngkg-query-shard -o json | jq -e '
  ...
  (.spec.template.spec.containers[0].volumeMounts |
    any(.name == "shuffle-spill" and .mountPath == "/var/lib/ngkg/shuffle")) and
  (.spec.template.spec.volumes |
    any(.name == "shuffle-spill" and (.emptyDir.sizeLimit | length > 0))) and
  (.spec.template.spec.containers[0].env |
    any(.name == "NGKG_SHUFFLE_SPILL_ROOT" and .value == "/var/lib/ngkg/shuffle")) and
  (.spec.template.spec.containers[0].env |
    any(.name == "NGKG_MAX_SHUFFLE_REQUEST_BYTES")) and
  (.spec.template.spec.containers[0].env |
    any(.name == "NGKG_FRAGMENT_ARROW_HTTP_CHUNK_BYTES")) and
  (.spec.template.spec.containers[0].env |
    any(.name == "NGKG_FRAGMENT_ARROW_CHANNEL_CAPACITY")) and
  (.spec.template.spec.containers[0].env |
...
jq -n \
  --argjson coordinatorRequestBytes "$(jq -r '.execution.coordinatorRequestBytes' "${run_dir}/response.json")" \
  --argjson streamedRequestsDelta "$(( requests_after - requests_before ))" \
  --argjson streamedBytesDelta "$(( bytes_after - bytes_before ))" \
  --arg evidenceDirectory "${run_dir}" \
  '{status: "phase32-coordinator-streaming-passed", exactIndependentBagEqual: true,
    coordinatorRequestMode: "streamed_from_spill_v1",
    coordinatorRequestBytes: $coordinatorRequestBytes,
    streamedRequestMetricDelta: $streamedRequestsDelta,
    streamedByteMetricDelta: $streamedBytesDelta,
    requestSpoolActiveBytesAfter: 0, graceSpillActiveBytesAfter: 0,
    evidenceDirectory: $evidenceDirectory,
    qualificationBoundary: "Also run pinned Rust, corruption, slow-consumer, disconnect, cancellation, disk-full, sustained concurrency/RSS, pod-loss, Helm dry-run, service-mesh and RKE2 79/80-percent scaling gates before release."}'
