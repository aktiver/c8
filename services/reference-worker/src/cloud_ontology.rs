...
use ngkg_ontology_qualifier::{
    ...
    assemble_snapshot_ontology, build_hermit_request, execute_hermit, finalize_qualification,
    project_partition, sha256_path,
};
...
pub async fn execute_project(
    ...
    let reference = context
        ...
        .ok_or_else(|| {
            CloudOntologyError::Config("partition is absent from semantic root".to_owned())
        })?;
    ...
    let facts = partition
        ...
        .ok_or_else(|| {
            CloudOntologyError::Config("semantic partition lacks canonical facts.nq".to_owned())
        })?;
...
pub async fn execute_assemble(
    ...
    let concurrency = required_usize(options, "ontology-download-concurrency")
        .map_err(CloudOntologyError::Config)?;
    if concurrency == 0 {
        return Err(CloudOntologyError::Config(
            "download concurrency must be positive".to_owned(),
        ));
    }
    ...
    for partition in 0..context.root.logical_partitions {
        ...
        context
            ...
            .materialize_unverified_bounded(
                &format!(
                    "{}/projections/{partition:05}/ontology-projection.json",
                    context.prefix
                ),
                manifest_limit,
                &manifest_path,
        ...
        for artifact in manifest.artifacts {
            if artifact.bytes > artifact_limit {
                return Err(CloudOntologyError::Config(
                    "ontology projection artifact exceeds ceiling".to_owned(),
                ));
            }
            downloads.push((
                format!(
                    "{}/projections/{partition:05}/{}",
    ...
    stream::iter(downloads.into_iter().map(|(key, sha256, bytes, path)| {
        ...
        async move {
            store
                .materialize_verified(&key, &sha256, bytes, &path)
                .await?;
    ...
    for (ordinal, import) in context.request.pinned_imports.iter().enumerate() {
        if import.bytes > artifact_limit {
            return Err(CloudOntologyError::Config(
                "pinned import exceeds ontology artifact ceiling".to_owned(),
            ));
        }
        let path = context.scratch.join(format!("pinned/{ordinal:08}.rdf"));
        context
            .store
            .materialize_verified(&import.object_key, &import.sha256, import.bytes, &path)
            .await?;
    ...
    upload_tree(
        &context.store,
        &output,
        &format!("{}/assembly", context.prefix),
        options,
    )
    .await?;
    ...
    patch_status(
        ...
        NgkgSourceImportStatus {
            ontology_assembly_object_key: Some(format!(
                "{}/assembly/ontology-assembly.json",
                context.prefix
            )),
...
pub async fn execute_qualify(
    ...
    reject_unknown(
        ...
        &[
            ...
            "ontology-qualification-request-object-key",
            "ontology-qualification-request-sha256",
            "ontology-assembly-object-key",
            "ontology-assembly-sha256",
            "ontology-max-manifest-bytes",
            "ontology-max-artifact-bytes",
            "java-executable",
    ...
    if required_value(options, "reasoner-name").map_err(CloudOntologyError::Config)? != "HermiT"
        || required_value(options, "reasoner-version").map_err(CloudOntologyError::Config)?
            != "1.4.5.519"
    {
        return Err(CloudOntologyError::Config(
            "qualification requires pinned HermiT 1.4.5.519".to_owned(),
        ));
    }
    let context = Context::new(options).await?;
    let assembly_key = required_value(options, "ontology-assembly-object-key")
        .map_err(CloudOntologyError::Config)?;
    let assembly_sha256 =
        required_value(options, "ontology-assembly-sha256").map_err(CloudOntologyError::Config)?;
    ...
    for document in &assembly.documents {
        if document.bytes > artifact_limit {
            return Err(CloudOntologyError::Config(
                "assembled ontology document exceeds ceiling".to_owned(),
            ));
        }
        context
            .store
    ...
    for path in collect_files(&reasoner_output)? {
        if fs::metadata(&path)?.len() > artifact_limit {
            return Err(CloudOntologyError::Config(
                "reasoner output exceeds ontology artifact ceiling".to_owned(),
            ));
        }
    }
    ...
    upload_tree(
        &context.store,
        &reasoner_output,
        &format!("{}/reasoner", context.prefix),
        options,
    )
    .await?;
    upload_tree(
...
fn parent_object_key(key: &str) -> Result<String, CloudOntologyError> {
    let (parent, _) = key
        .rsplit_once('/')
        .ok_or_else(|| CloudOntologyError::Config("object key has no parent".to_owned()))?;
...
async fn load_status(
    options: &BTreeMap<String, String>,
) -> Result<NgkgSourceImportStatus, CloudOntologyError> {
...
async fn patch_status(
    ...
    let document = source_import_status_apply_document(
        ...
        &status,
        &[
            "ontologyAssemblyObjectKey", "ontologyAssemblySha256",
            "ontologyQualificationRootObjectKey", "ontologyQualificationRootSha256",
        ],
    )?;
    api.patch_status(
        &name,
        &PatchParams::apply("ngkg-ontology-worker"),
