...
use ngkg_catalog::{CloudSnapshotActivation, CommitCloudSnapshotActivation, OperationRepository};
use ngkg_kube::{
...
use ngkg_snapshot_activation::{
    ActivationInputs, ActivationRootReference, SnapshotActivationManifest, build_serving_artifacts,
};
...
const ALLOWED: &[&str] = &[
    ...
    "artifact-base-url",
    "scratch-root",
    "semantic-compilation-root-object-key",
    "semantic-compilation-root-sha256",
    "ontology-qualification-request-object-key",
    "ontology-qualification-request-sha256",
    "ontology-qualification-root-object-key",
    "ontology-qualification-root-sha256",
    "offline-reasoning-root-object-key",
    "offline-reasoning-root-sha256",
...
pub async fn execute(options: &BTreeMap<String, String>) -> Result<String, CloudActivationError> {
    ...
    let semantic: SemanticCompilationRoot = materialize_json(
        ...
        &semantic_ref,
        manifest_limit,
        &scratch.join("semantic-root.json"),
    )
    .await?;
    let qualification_request: OntologyQualificationRequest = materialize_json(
        ...
        &request_ref,
        manifest_limit,
        &scratch.join("qualification-request.json"),
    )
    .await?;
    let qualification: OntologyQualificationRoot = materialize_json(
        ...
        &qualification_ref,
        manifest_limit,
        &scratch.join("qualification-root.json"),
    )
    .await?;
    let offline: OfflineReasoningRoot = materialize_json(
        ...
        &offline_ref,
        manifest_limit,
        &scratch.join("offline-root.json"),
    )
    .await?;
    ...
    catalog.ready().await?;
    let durable = catalog
        .get_compilation(semantic.tenant_id, semantic.operation_id)
        .await?;
    ...
    let facts = materialize_semantic_partitions(
        ...
        &scratch,
        manifest_limit,
        limit(options, "activation-max-partition-bytes")?,
        concurrency,
    )
    .await?;
    ...
    let qualification_prefix = parent_object_key(&inputs.qualification_root_ref.object_key)?
        .strip_suffix("/root")
        .ok_or_else(|| {
            CloudActivationError::Config("qualification root key is noncanonical".to_owned())
        })?
        .to_owned();
    ...
    let reasoner_files = [
        ...
        (
            "owl-profile-qualification.json",
            &qualification.owl_profile_qualification_sha256,
        ),
        (
            "owl-consistency-qualification.json",
            &qualification.owl_consistency_qualification_sha256,
        ),
    ];
    for (name, sha256) in reasoner_files {
        store
            .materialize_verified(
                &format!("{qualification_prefix}/reasoner/{name}"),
                sha256,
                limit(options, "activation-max-partition-bytes")?,
                &reasoner_root.join(name),
            )
            .await?;
    }
    ...
    let artifacts = build_serving_artifacts(
        &inputs,
        &facts,
        &reasoner_root.join("finite-closure.nt"),
        &reasoner_root.join("owl-signature.json"),
        &datatype_policy,
        &reasoner_root.join("owl-profile-qualification.json"),
        &reasoner_root.join("owl-consistency-qualification.json"),
        &output,
    )?;
    ...
    let outcome = catalog
        .commit_cloud_snapshot_activation(
            semantic.tenant_id,
            ...
            &CommitCloudSnapshotActivation {
                activation: CloudSnapshotActivation {
                    ...
                    query_dataset_bytes: i64::try_from(activation.query_dataset_bytes).map_err(
                        |_| CloudActivationError::Config("query dataset size overflow".to_owned()),
                    )?,
                    fact_count: i64::try_from(activation.fact_count).map_err(|_| {
                        CloudActivationError::Config("fact count overflow".to_owned())
                    })?,
                    consequence_count: i64::try_from(activation.consequence_count).map_err(
                        |_| CloudActivationError::Config("consequence count overflow".to_owned()),
                    )?,
                    semantic_partition_count: i32::try_from(activation.semantic_partition_count)
                        .map_err(|_| {
                            CloudActivationError::Config(
                                "semantic partition count overflow".to_owned(),
                            )
                        })?,
                    reasoning_partition_count: i32::try_from(activation.reasoning_partition_count)
                        .map_err(|_| {
                            CloudActivationError::Config(
                                "reasoning partition count overflow".to_owned(),
                            )
                        })?,
                },
    ...
    patch_status(
        ...
        NgkgSourceImportStatus {
            snapshot_activation_manifest_object_key: Some(activation_key.clone()),
            snapshot_activation_manifest_sha256: Some(activation_sha256.clone()),
...
async fn materialize_semantic_partitions(
    ...
    let jobs = root.partitions.iter().map(|reference| {
        ...
        async move {
            ...
            store
                .materialize_verified(&manifest_key, &manifest_sha, manifest_limit, &manifest_path)
                .await?;
            let manifest: SemanticPartitionManifest = read_json(&manifest_path)?;
            let facts = manifest
                .artifacts
                .iter()
                .find(|artifact| artifact.relative_path == "facts.nq")
                .ok_or_else(|| {
                    CloudActivationError::Config("semantic partition lacks facts.nq".to_owned())
                })?;
            if facts.bytes > artifact_limit {
                return Err(CloudActivationError::Config(
                    "semantic facts partition exceeds ceiling".to_owned(),
                ));
            }
    ...
    {
        return Err(CloudActivationError::Config(
            "semantic partition materialization is incomplete".to_owned(),
        ));
    }
...
async fn verify_offline_partitions(
    ...
    for reference in &root.partitions {
        ...
        {
            return Err(CloudActivationError::Config(
                "offline partition identity mismatch".to_owned(),
            ));
        }
        ...
        for artifact in manifest.artifacts {
            if artifact.bytes > artifact_limit {
                return Err(CloudActivationError::Config(
                    "offline artifact exceeds ceiling".to_owned(),
                ));
            }
            checks.push((
...
async fn materialize_json<T: DeserializeOwned>(
    store: &ArtifactStore,
    reference: &ActivationRootReference,
    limit: u64,
    path: &Path,
) -> Result<T, CloudActivationError> {
    store
        .materialize_verified(&reference.object_key, &reference.sha256, limit, path)
        .await?;
    read_json(path)
}
...
fn root_ref(
    ...
    prefix: &str,
) -> Result<ActivationRootReference, CloudActivationError> {
    Ok(ActivationRootReference {
        object_key: required_value(options, &format!("{prefix}-object-key"))
            .map_err(CloudActivationError::Config)?,
        sha256: required_value(options, &format!("{prefix}-sha256"))
            .map_err(CloudActivationError::Config)?,
    })
}

async fn qualification_request_parent(
    options: &BTreeMap<String, String>,
) -> Result<Option<uuid::Uuid>, CloudActivationError> {
    ...
    Ok(api
        .get(&required_value(options, "import-name").map_err(CloudActivationError::Config)?)
        .await?
        .spec
        .parent_snapshot_id)
}
...
fn decode_sha256(value: &str) -> Result<[u8; 32], CloudActivationError> {
    let bytes = hex::decode(value)
        .map_err(|_| CloudActivationError::Config("invalid SHA-256".to_owned()))?;
    bytes
        .try_into()
        .map_err(|_| CloudActivationError::Config("invalid SHA-256 length".to_owned()))
}
fn sha256_path(path: &Path) -> Result<String, CloudActivationError> {
    ngkg_offline_reasoner::sha256_path(path)
        .map_err(|error| CloudActivationError::Config(error.to_string()))
}
fn parent_object_key(key: &str) -> Result<String, CloudActivationError> {
    let (parent, _) = key
        .rsplit_once('/')
        .ok_or_else(|| CloudActivationError::Config("object key has no parent".to_owned()))?;
...
fn collect_files(root: &Path) -> Result<Vec<PathBuf>, CloudActivationError> {
    ...
    while let Some(directory) = pending.pop() {
        for entry in fs::read_dir(directory)? {
            ...
            if kind.is_symlink() {
                return Err(CloudActivationError::Config(
                    "activation output contains a symlink".to_owned(),
                ));
            }
            if kind.is_dir() {
...
async fn upload_tree(
    ...
    for path in collect_files(root)? {
        let relative = path
            .strip_prefix(root)
            .map_err(|_| CloudActivationError::Config("activation output escaped root".to_owned()))?
            .to_str()
            .ok_or_else(|| {
                CloudActivationError::Config("activation output path is not UTF-8".to_owned())
            })?;
        store
            .put_file_immutable(
...
async fn load_status(
    options: &BTreeMap<String, String>,
) -> Result<NgkgSourceImportStatus, CloudActivationError> {
    let api: Api<NgkgSourceImport> = Api::namespaced(
        Client::try_default().await?,
        &required_value(options, "namespace").map_err(CloudActivationError::Config)?,
    );
...
async fn patch_status(
    ...
    let document = source_import_status_apply_document(
        ...
        &status,
        &[
            "snapshotActivationManifestObjectKey", "snapshotActivationManifestSha256",
            "snapshotPublicationState",
        ],
    )?;
    api.patch_status(
        &name,
        &PatchParams::apply("ngkg-snapshot-activation-worker"),
