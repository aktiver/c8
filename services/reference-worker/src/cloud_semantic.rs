...
use ngkg_semantic_compiler::{
    CompilerHandoffManifest, DictionaryManifest, FragmentMapManifest, MapLimits,
    ReduceLimits, SemanticCompilationRoot, SemanticPartitionManifest, finalize_committed_semantic_root,
...
pub async fn execute_map(
    ...
    reject_unknown(options, &[
        ...
        "compiler-handoff-object-key", "compiler-handoff-sha256",
        "semantic-max-manifest-bytes", "single-put-max-bytes",
        "multipart-buffer-bytes", "multipart-concurrency", "completion-index",
        "semantic-max-fragment-bytes", "semantic-max-fragment-quads",
        "semantic-map-rows-in-memory", "semantic-map-worker-threads",
...
pub async fn execute_dictionary(
    ...
    reject_unknown(options, &[
        ...
        "compiler-handoff-object-key", "compiler-handoff-sha256",
        "semantic-max-manifest-bytes", "single-put-max-bytes",
        "multipart-buffer-bytes", "multipart-concurrency", "semantic-max-run-bytes",
    ...
    let dictionary_limit = required_u64(options, "semantic-max-dictionary-bytes")
        .map_err(CloudSemanticError::Config)?;
    for file in [output.join("dictionary.tsv"), output.join("guid-dictionary.tsv")] {
    ...
    upload_tree(&context, &output, &format!("{}/dictionary", context.prefix), options).await?;
...
pub async fn execute_partition(
    ...
    reject_unknown(options, &[
        ...
        "semantic-max-manifest-bytes", "single-put-max-bytes",
        "multipart-buffer-bytes", "multipart-concurrency", "completion-index",
        "semantic-max-run-bytes", "semantic-max-dictionary-bytes",
        "semantic-max-input-runs", "semantic-max-partition-quads",
...
pub async fn execute_finalize(
    ...
    reject_unknown(options, &[
        ...
        "compiler-handoff-object-key", "compiler-handoff-sha256",
        "semantic-max-manifest-bytes", "single-put-max-bytes",
        "multipart-buffer-bytes", "multipart-concurrency",
        "semantic-max-dictionary-bytes", "semantic-max-artifact-bytes",
    ...
    context
        ...
        .await?;
    let manifest_limit = required_u64(options, "semantic-max-manifest-bytes")
        .map_err(CloudSemanticError::Config)?;
    let artifact_limit = required_u64(options, "semantic-max-artifact-bytes")
        .map_err(CloudSemanticError::Config)?;
    ...
    for partition in 0..context.handoff.logical_partitions {
        ...
        context
            .store
            .materialize_unverified_bounded(
                &format!("{}/partitions/{partition:05}/semantic-partition.json", context.prefix),
                manifest_limit,
                &path,
            )
            .await?;
    ...
    upload_tree(&context, &output, &format!("{}/root", context.prefix), options).await?;
...
async fn materialize_maps(
    ...
    let manifest_limit = required_u64(options, "semantic-max-manifest-bytes")
        .map_err(CloudSemanticError::Config)?;
    let run_limit = required_u64(options, "semantic-max-run-bytes")
        .map_err(CloudSemanticError::Config)?;
    ...
    for ordinal in 0..context.handoff.total_objects {
        ...
        let artifacts = if let Some(partition) = partition {
            manifest.fact_runs.get(&partition).cloned().unwrap_or_default()
        } else {
