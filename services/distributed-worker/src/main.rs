...
use ngkg_distributed_build::{
    SafeScanRequest, compare_roots, finalize_reducers, project_partition,
    reduce_projection_runs, safe_scan_trig,
};
...
use ngkg_semantic_compiler::{
    CompilerHandoffManifest, MapLimits, ReduceLimits, finalize_dictionary,
    finalize_semantic_root, map_fragment, reduce_partition,
};
...
async fn run() -> Result<String, String> {
    ...
    match command.as_str() {
        ...
        "plan-object-store" => object_stage::plan(&options).await.map_err(|error| error.to_string()),
        "project-object-store" => object_stage::project(&options).await.map_err(|error| error.to_string()),
        "reduce-object-store" => object_stage::reduce(&options).await.map_err(|error| error.to_string()),
        "finalize-object-store" => object_stage::finalize(&options).await.map_err(|error| error.to_string()),
        "prepare-artifacts-object-store" => object_stage::prepare_artifacts(&options)
            .await
            .map_err(|error| error.to_string()),
        "materialize-artifact-object-store" => object_stage::materialize_artifact_object_store(&options)
            .await
            .map_err(|error| error.to_string()),
        "finalize-artifacts-object-store" => object_stage::finalize_artifacts_object_store(&options)
            .await
            .map_err(|error| error.to_string()),
        "prepare-serving-root-object-store" => object_stage::prepare_serving_root_object_store(&options)
            .await
            .map_err(|error| error.to_string()),
        _ => Err(usage()),
    }
...
fn semantic_map(options: &BTreeMap<String, String>) -> Result<String, String> {
    ...
    let handoff: CompilerHandoffManifest = serde_json::from_slice(
        &std::fs::read(&handoff_path).map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
...
fn compile_mmap_locator(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &["locator", "locator-sha256", "snapshot-id", "output"])?;
...
fn materialize_artifact(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source-plan", "source-plan-sha256", "dictionary", "dictionary-sha256",
        "partition-index", "dataset-namespace", "source-guid", "source-snapshot",
        "source-sha256", "projection-policy", "projection-policy-sha256", "output-root",
        "max-quads", "row-group-rows",
    ])?;
    let policy_path = path(options, "projection-policy")?;
    verify_hash(&policy_path, &value(options, "projection-policy-sha256")?)?;
    let policy: ProjectionPolicy = serde_json::from_slice(
        &std::fs::read(&policy_path).map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
...
fn finalize_artifacts(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source-plan", "source-plan-sha256", "dictionary", "dictionary-sha256",
        "artifact-manifest-list", "output-root",
    ])?;
...
fn safe_scan(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source", "source-sha256", "projection-policy", "projection-policy-sha256",
        "output-root", "dataset-id", "snapshot-id", "dataset-namespace", "source-guid",
        "source-snapshot", "logical-partitions", "max-quads",
    ])?;
    let policy_path = path(options, "projection-policy")?;
    verify_hash(&policy_path, &value(options, "projection-policy-sha256")?)?;
    let policy: ProjectionPolicy = serde_json::from_slice(
    ...
    Ok(serde_json::json!({"status":"source-planned","sourcePlan":plan,"sourcePlanSha256":sha}).to_string())
}

fn project(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source-plan", "source-plan-sha256", "partition-index", "dataset-namespace",
        "source-guid", "source-snapshot", "projection-policy", "projection-policy-sha256",
        "output-root", "max-quads",
    ])?;
    let policy_path = path(options, "projection-policy")?;
    verify_hash(&policy_path, &value(options, "projection-policy-sha256")?)?;
    let policy: ProjectionPolicy = serde_json::from_slice(
...
fn reduce(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source-plan", "source-plan-sha256", "projection-manifest-list", "reducer-index",
        "reducer-count", "output-root",
    ])?;
...
fn finalize(options: &BTreeMap<String, String>) -> Result<String, String> {
    reject_unknown(options, &[
        "source-plan", "source-plan-sha256", "reducer-manifest-list", "output-root",
    ])?;
...
fn compare(options: &BTreeMap<String, String>) -> Result<String, String> {
    ...
    if !report.equivalent {
        return Err(format!("builds are not equivalent: {}", report.mismatches.join("; ")));
    }
    serde_json::to_string(&report).map_err(|error| error.to_string())
}
...
fn manifest_list(path: &PathBuf) -> Result<Vec<PathBuf>, String> {
    ...
    values.into_iter().map(|value| {
        if value.is_empty() { Err("manifest list contains an empty path".to_owned()) }
        else { Ok(PathBuf::from(value)) }
    }).collect()
}
...
fn parse_options(arguments: Vec<String>) -> Result<BTreeMap<String, String>, String> {
    ...
    for pair in arguments.chunks_exact(2) {
        let name = pair[0].strip_prefix("--").filter(|value| !value.is_empty())
...
fn value(options: &BTreeMap<String, String>, name: &str) -> Result<String, String> {
    options.get(name).filter(|value| !value.is_empty()).cloned()
...
fn uuid(options: &BTreeMap<String, String>, name: &str) -> Result<Uuid, String> {
    value(options, name)?.parse::<Uuid>().map_err(|_| format!("--{name} must be a UUID"))
}

fn positive_u64(options: &BTreeMap<String, String>, name: &str) -> Result<u64, String> {
    value(options, name)?.parse::<u64>().ok().filter(|value| *value > 0)
        .ok_or_else(|| format!("--{name} must be a positive integer"))
}

fn positive_u32(options: &BTreeMap<String, String>, name: &str) -> Result<u32, String> {
    value(options, name)?.parse::<u32>().ok().filter(|value| *value > 0)
...
fn positive_usize(options: &BTreeMap<String, String>, name: &str) -> Result<usize, String> {
    value(options, name)?.parse::<usize>().ok().filter(|value| *value > 0)
...
fn nonnegative_u32(options: &BTreeMap<String, String>, name: &str) -> Result<u32, String> {
    value(options, name)?.parse::<u32>().map_err(|_| format!("--{name} must be a non-negative 32-bit integer"))
}

fn verify_hash(path: &PathBuf, expected: &str) -> Result<(), String> {
