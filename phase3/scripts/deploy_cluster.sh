...
[[ "${NGKG_PROVIDER}" =~ ^(rke|rke2|eks|aks|gke)$ ]] || die "unsupported provider"
...
read -r hpc_worker_repo hpc_worker_digest <<<"$(image ngkg-hpc-worker)"
...
platform_set=(
  ...
  --set-string "images.hpcWorker.repository=${hpc_worker_repo}" --set-string "images.hpcWorker.digest=${hpc_worker_digest}"
...
server_version="$(kubectl --context "${NGKG_KUBECTL_CONTEXT}" version -o json | jq -er '.serverVersion.gitVersion | ltrimstr("v")')"
helm template ngkg-crds "${candidate_root}/NGKG_1_0_0_GA/charts/ngkg-crds" --namespace "${NGKG_NAMESPACE}" --kube-version "${server_version}" >>"${rendered}"
helm template ngkg-platform "${candidate_root}/NGKG_1_0_0_GA/charts/ngkg-platform" --namespace "${NGKG_NAMESPACE}" --kube-version "${server_version}" --values "${NGKG_PLATFORM_VALUES}" "${platform_set[@]}" >>"${rendered}"
helm template ngkg-workloads "${candidate_root}/NGKG_1_0_0_GA/charts/ngkg-workloads" --namespace "${NGKG_NAMESPACE}" --kube-version "${server_version}" --values "${NGKG_WORKLOAD_VALUES}" "${workload_set[@]}" >>"${rendered}"
helm template ngkg-agents "${candidate_root}/ngkg-agents/charts/ngkg-agents" --namespace "${NGKG_NAMESPACE}" --kube-version "${server_version}" --values "${NGKG_AGENT_VALUES}" "${agent_set[@]}" >>"${rendered}"
