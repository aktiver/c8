...
def main() -> int:
    ...
    require(catalog["formatVersion"] == 1 and len(names) == 13 and len(set(names)) == 13, "image catalog must contain thirteen unique Phase 8 images")
    require({item["kind"] for item in catalog["images"]} == {"build"}, "all images must have Dockerfiles under docker_repos")
    require(names[-1] == "ngkg-vllm" and catalog["images"][-1]["sourceEnvironment"] == "NGKG_VLLM_SOURCE_IMAGE", "vLLM must be rebuilt from a digest-pinned source")
    require(all(row.get("context") == "." and str(row.get("dockerfile", "")).startswith("docker_repos/") for row in catalog["images"]), "all release Dockerfiles must live under docker_repos")

    for item in catalog["images"]:
        if item["kind"] != "build":
            continue
        dockerfile = text(f"{item['context']}/{item['dockerfile']}")
        if item["name"] != "ngkg-vllm":
            require("ARG BUILD_OFFLINE=false" in dockerfile, f"selectable online/offline build mode missing: {item['name']}")
            require('true) set -- --offline' in dockerfile, f"offline Cargo mode missing: {item['name']}")
            require('cargo build --locked "$@" --release' in dockerfile, f"locked Cargo release build missing: {item['name']}")
        if "hermit-reasoner" in dockerfile:
            require('mvn --batch-mode --no-transfer-progress "$@"' in dockerfile, f"selectable offline Maven build missing: {item['name']}")
        if item["name"] != "ngkg-vllm":
    ...
    for marker in ("docker buildx build", "--network=none", "BUILD_OFFLINE=true", "MPI_BUILDER_IMAGE", "MPI_RUNTIME_IMAGE", "--provenance=mode=max", "--sbom=true", "syft", "grype", "trivy", "cosign sign", "cosign attest", "cosign verify", "require_digest_ref"):
    ...
    require(set(scenarios["providers"]) == {"rke", "rke2", "eks", "aks", "gke"}, "provider matrix is incomplete")
    ...
    for marker in ("helm upgrade --install", "--atomic", "--wait-for-jobs", "imageLockSha256", "renderedManifestSha256", "images.hpcWorker.repository", "images.reasoner.repository", "vllm.image.repository", "serverVersion.gitVersion"):
        require(marker in deploy, f"atomic deployment control missing: {marker}")

    issuer = text("phase3/scripts/verify_and_issue.py")
    for marker in ("cosign", "verify-attestation", "verify-blob", "thirteen unique deployable images", "provider evidence matrix is incomplete", "scenario coverage mismatch", "certificateSha256"):
    ...
    for marker in ("permissions:", "id-token: write", "environment: ngkg-phase3-release", "NGKG_MPI_BUILDER_IMAGE", "NGKG_MPI_RUNTIME_IMAGE", "build_supply_chain.sh", "qualify_postgres.sh", "deploy_cluster.sh", "qualify_cluster.py", "verify_and_issue.py", "matrix:", "rke2", "eks", "aks", "gke"):
