...
: "${NGKG_MPI_BUILDER_IMAGE:?digest-pinned MPI/OpenMP builder is required}"
: "${NGKG_MPI_RUNTIME_IMAGE:?digest-pinned MPI runtime is required}"
...
require_digest_ref NGKG_MPI_BUILDER_IMAGE "${NGKG_MPI_BUILDER_IMAGE}"
require_digest_ref NGKG_MPI_RUNTIME_IMAGE "${NGKG_MPI_RUNTIME_IMAGE}"
...
while IFS= read -r item; do
  ...
  if [[ "${kind}" == build ]]; then
    ...
    docker buildx build --pull=false --network=none --push \
      ...
      --build-arg "VLLM_SOURCE_IMAGE=${NGKG_VLLM_SOURCE_IMAGE}" \
      --build-arg "MPI_BUILDER_IMAGE=${NGKG_MPI_BUILDER_IMAGE}" \
      --build-arg "MPI_RUNTIME_IMAGE=${NGKG_MPI_RUNTIME_IMAGE}" \
  ...
  python3 "${phase3_root}/scripts/write_provenance.py" \
    ...
    --base "${NGKG_MPI_BUILDER_IMAGE}" --base "${NGKG_MPI_RUNTIME_IMAGE}" \
    --output "${provenance}"
