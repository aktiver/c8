...
def canonical_graph(payload: bytes, canonicalizer: Path | None, canonicalizer_sha256: str | None) -> tuple[str, dict[str, Any]]:
    require(canonicalizer is not None, "RDF graph comparison requires a pinned RDFC-1.0 canonicalizer")
...
def endpoint_result(
    ...
    expected_types = {item.split(";", 1)[0].strip().lower() for item in accept.split(",")}
    require(content_type in expected_types, f"endpoint violated content negotiation: {content_type}")
...
def main() -> int:
    ...
    native_endpoint = str(config.get("nativeEndpoint", "")).rstrip("/")
    oracle_endpoint = str(config.get("oracleEndpoint", "")).rstrip("/")
    require(native_endpoint and oracle_endpoint and native_endpoint != oracle_endpoint, "native and oracle endpoints must be distinct")
    ...
    for case in cases:
        ...
        for repetition in range(minimum_repetitions):
            native = endpoint_result(native_endpoint, native_token, query, accept, maximum, timeout_seconds, ssl_context)
            oracle = endpoint_result(oracle_endpoint, oracle_token, query, accept, maximum, timeout_seconds, ssl_context)
            ...
            require(reported_hash == native_hash, "native reported semantic hash does not match independently canonicalized bytes")
            oracle_reported_hash = oracle["headers"].get("x-ngkg-semantic-result-sha256")
            require(valid_sha256(oracle_reported_hash) and oracle_reported_hash == oracle_hash, "oracle reported semantic hash is invalid")
            require(
            ...
            require(oracle["headers"].get("x-ngkg-snapshot-id") == observed_snapshot, "native and oracle evaluated different snapshots")
            for identity_header in (
                "x-ngkg-authorized-graph-set-sha256",
                "x-ngkg-datatype-policy-sha256",
                "x-ngkg-ontology-import-set-sha256",
                "x-ngkg-query-sha256",
            ):
                native_identity = native["headers"].get(identity_header)
                require(valid_sha256(native_identity), f"native response omitted {identity_header}")
                require(oracle["headers"].get(identity_header) == native_identity, f"native/oracle {identity_header} mismatch")
            expected_route = case["reasoningRoute"]
            require(native["headers"].get("x-ngkg-reasoning-route") == expected_route, "native reasoning route mismatch")
            require(oracle["headers"].get("x-ngkg-reasoning-route") == expected_route, "oracle reasoning route mismatch")
