...
Enterprise Stabilization Phase 6 builds on the dependency-isolated native runtime with production differential, multinode capacity, saturation, chaos and signed release-evidence closure. Successful SPARQL Protocol responses expose the active native cutover mode and canonical full-result hash; the controlled workflow requires `required` mode, exact semantic equality, measured physical resources, four-provider recovery and signed supply-chain evidence. See `ENTERPRISE_STABILIZATION_PHASE_6_DELIVERY_REPORT.md` and `BUILD_STATUS_ENTERPRISE_STABILIZATION_PHASE_6.md`.

For the shortest HA Kubernetes setup and first direct TriG upload, start with [`QUICKSTART.md`](QUICKSTART.md).
