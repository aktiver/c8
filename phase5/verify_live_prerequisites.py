...
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1] / "phase6/scripts"))
from evidence_security import verify_signed_statement

REQUIRED = {
    "phase3-certificate.json": {
        "oci_supply_chain", "postgres_ha", "rke", "rke2", "eks", "aks", "gke",
...
def verify(path: pathlib.Path, required: set[str], subject: str, identity: str, issuer: str, cosign: str) -> None:
    bundle = path.with_name(path.name.removesuffix(".json") + ".sigstore.json")
    document = verify_signed_statement(path, bundle, subject=subject, identity_regexp=identity, oidc_issuer=issuer, cosign=cosign)
...
def main() -> int:
    ...
    for name, scenarios in REQUIRED.items():
        ...
        verify(path, scenarios, args.subject_sha256, args.certificate_identity_regexp, args.certificate_oidc_issuer, args.cosign)
