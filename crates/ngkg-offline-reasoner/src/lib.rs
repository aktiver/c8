...
pub fn validate_qualification(root: &OntologyQualificationRoot) -> Result<(), OfflineReasoningError> {
...
pub fn plan_exact_consequences(
    ...
    if sha256_path(finite_closure_path)? != qualification_root.finite_closure_sha256 {
        return Err(OfflineReasoningError::Contract("finite closure checksum mismatch".to_owned()));
    }
    if limits.logical_partitions == 0 || limits.rows_in_memory == 0 || limits.max_run_bytes == 0 {
        return Err(OfflineReasoningError::Contract("planner ceilings must be positive".to_owned()));
    }
    ...
    let mut spools = (0..shard_count).map(|index| {
    ...
    for parsed in parser {
        ...
        if parsed_count > limits.max_consequences {
            return Err(OfflineReasoningError::Contract("HermiT consequence ceiling exceeded".to_owned()));
        }
        ...
        let shard = usize::try_from(partition % shard_count)
            .map_err(|_| OfflineReasoningError::Contract("planner shard index overflow".to_owned()))?;
        writeln!(spools[shard], "{partition:010}\t{canonical}")?;
    }
    for spool in &mut spools { spool.flush()?; spool.get_ref().sync_all()?; }
    drop(spools);
    for shard in 0..shard_count {
        sort_spool_into_partition_runs(
            &spool_root.join(format!("shard-{shard:04}.spool")), output_root,
            limits.rows_in_memory, limits.max_run_bytes, &mut runs,
        )?;
    }
...
pub fn reduce_exact_partition(
    ...
    if partition_index >= plan.logical_partitions || row_group_rows == 0 {
        return Err(OfflineReasoningError::Contract("invalid reducer partition or row-group ceiling".to_owned()));
    }
    create_new_root(output_root)?;
    let expected_runs = plan.partition_runs.get(&partition_index).cloned().unwrap_or_default();
    ...
    for run in &expected_runs {
        ...
        if fs::metadata(&path)?.len() != run.bytes {
            return Err(OfflineReasoningError::Contract("run byte count mismatch".to_owned()));
        }
    ...
    let semantic_content_sha256 = semantic_digest(&canonical, &output_root.join("proof-support.tsv"))?;
...
pub fn finalize_offline_reasoning(
    ...
    for path in partition_manifests {
        ...
        let parent = path.parent().ok_or_else(|| OfflineReasoningError::Contract("manifest has no parent".to_owned()))?;
        for artifact in &manifest.artifacts { verify_file(&parent.join(&artifact.relative_path), &artifact.sha256)?; }
    ...
    finalize_offline_reasoning_verified(
        plan_path, plan_sha256, qualification_root, qualification_root_sha256, &verified, output_root,
    )
}
...
pub fn finalize_offline_reasoning_verified(
    ...
    let plan: OfflineReasoningPlan = read_json(plan_path)?;
    validate_plan(&plan, qualification_root, qualification_root_sha256)?;
    if partitions.len() != usize::try_from(plan.logical_partitions)
        .map_err(|_| OfflineReasoningError::Contract("logical partition count overflow".to_owned()))?
    {
        return Err(OfflineReasoningError::Contract("partition completion barrier is incomplete".to_owned()));
    }
    create_new_root(output_root)?;
    ...
    for input in &ordered {
        ...
        if index >= seen.len() || seen[index] {
            return Err(OfflineReasoningError::Contract("duplicate or out-of-range partition completion".to_owned()));
        }
        ...
        {
            return Err(OfflineReasoningError::Contract("partition completion identity mismatch".to_owned()));
        }
        seen[index] = true;
        consequence_count = checked_add(consequence_count, manifest.consequence_count, "consequence")?;
        proof_root.update(manifest.semantic_content_sha256.as_bytes());
        let same_as = manifest.artifacts.iter().find(|artifact| artifact.relative_path == "sameas-membership.tsv")
            .ok_or_else(|| OfflineReasoningError::Contract("partition lacks equality membership artifact".to_owned()))?;
    ...
    if seen.iter().any(|value| !*value) {
        return Err(OfflineReasoningError::Contract("missing partition completion".to_owned()));
    }
...
fn validate_plan(
    ...
    {
        return Err(OfflineReasoningError::Contract("offline plan identity mismatch".to_owned()));
    }
    Ok(())
}
...
fn sort_spool_into_partition_runs(
    ...
    for row in BufReader::new(File::open(spool)?).lines() {
        buffer.push(row?);
        if buffer.len() >= rows_in_memory { flush_sort_run(&scratch, &mut buffer, &mut chunks)?; }
    }
    ...
    let mut readers = chunks.iter().map(|path| File::open(path).map(BufReader::new)).collect::<Result<Vec<_>, _>>()?;
    let mut heap = BinaryHeap::new();
    for (index, reader) in readers.iter_mut().enumerate() {
        if let Some(row) = next_row(reader)? { heap.push(HeapRow { row, reader: index }); }
    }
    let mut previous = None::<String>;
    let mut active = None::<ActiveRun>;
    while let Some(item) = heap.pop() {
        if previous.as_deref() != Some(&item.row) {
            let (prefix, triple) = item.row.split_once('\t')
                .ok_or_else(|| OfflineReasoningError::Contract("planner spool row is malformed".to_owned()))?;
            let partition = prefix.parse::<u32>()
                .map_err(|_| OfflineReasoningError::Contract("planner partition is malformed".to_owned()))?;
            let row_bytes = u64::try_from(triple.len() + 1)
                .map_err(|_| OfflineReasoningError::Contract("planner row length overflow".to_owned()))?;
            if row_bytes > max_run_bytes {
                return Err(OfflineReasoningError::Contract("one consequence exceeds run byte ceiling".to_owned()));
            }
            let rotate = active.as_ref().is_none_or(|run| {
                run.partition != partition || run.bytes.saturating_add(row_bytes) > max_run_bytes
            });
            if rotate {
                if let Some(run) = active.take() { finish_active_run(run, runs)?; }
                active = Some(start_active_run(output_root, partition, runs.get(&partition).map_or(0, Vec::len))?);
            }
            let run = active.as_mut().ok_or_else(|| OfflineReasoningError::Contract("planner run is absent".to_owned()))?;
        ...
        if let Some(row) = next_row(&mut readers[item.reader])? { heap.push(HeapRow { row, reader: item.reader }); }
    }
    if let Some(run) = active { finish_active_run(run, runs)?; }
...
fn start_active_run(output_root: &Path, partition: u32, ordinal: usize) -> Result<ActiveRun, OfflineReasoningError> {
    let relative_path = format!("runs/{partition:05}/run-{ordinal:08}.nt");
    let path = output_root.join(&relative_path);
    if let Some(parent) = path.parent() { fs::create_dir_all(parent)?; }
    Ok(ActiveRun { partition, relative_path, writer: BufWriter::new(create_new(&path)?), path, bytes: 0, rows: 0 })
}

fn finish_active_run(mut run: ActiveRun, runs: &mut BTreeMap<u32, Vec<ConsequenceRun>>) -> Result<(), OfflineReasoningError> {
    run.writer.flush()?; run.writer.get_ref().sync_all()?; drop(run.writer);
    runs.entry(run.partition).or_default().push(ConsequenceRun {
        relative_path: run.relative_path, sha256: sha256_path(&run.path)?, bytes: run.bytes, row_count: run.rows,
    });
    Ok(())
}

fn merge_runs_hierarchical(inputs: &[PathBuf], scratch: &Path, output: &Path) -> Result<(), OfflineReasoningError> {
    if inputs.is_empty() {
    ...
    while generation.len() > MERGE_FAN_IN {
        ...
        if level > 0 {
            for path in generation { fs::remove_file(path)?; }
        }
        generation = next;
        level = level.checked_add(1).ok_or_else(|| OfflineReasoningError::Contract("merge level overflow".to_owned()))?;
    }
    merge_sorted_unique(&generation, output)?;
    if level > 0 { for path in generation { fs::remove_file(path)?; } }
    Ok(())
}

#[derive(Eq, PartialEq)]
struct HeapRow { row: String, reader: usize }
impl Ord for HeapRow { fn cmp(&self, other: &Self) -> std::cmp::Ordering { other.row.cmp(&self.row).then_with(|| other.reader.cmp(&self.reader)) } }
impl PartialOrd for HeapRow { fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> { Some(self.cmp(other)) } }

fn merge_sorted_unique(inputs: &[PathBuf], output: &Path) -> Result<(), OfflineReasoningError> {
    let mut readers = inputs.iter().map(|path| File::open(path).map(BufReader::new)).collect::<Result<Vec<_>, _>>()?;
    let mut heap = BinaryHeap::new();
    for (index, reader) in readers.iter_mut().enumerate() {
        if let Some(row) = next_row(reader)? { heap.push(HeapRow { row, reader: index }); }
    }
...
fn build_partition_artifacts(
    ...
    let properties = WriterProperties::builder().set_max_row_group_size(row_group_rows).build();
    ...
    let parser = RdfParser::from_format(RdfFormat::NTriples).for_reader(BufReader::new(File::open(canonical)?));
    for parsed in parser {
        let quad = parsed.map_err(|error| OfflineReasoningError::Rdf(error.to_string()))?;
        ...
        let without_dot = triple.strip_suffix(" .").ok_or_else(|| OfflineReasoningError::Contract("canonical triple lacks terminator".to_owned()))?;
        ...
        match quad.predicate.as_str() {
            ...
            RDFS_SUBCLASS | OWL_EQUIVALENT_CLASS => writeln!(class_hierarchy, "{}\t{}\t{support}", quad.subject, quad.object)?,
            RDFS_SUBPROPERTY | OWL_EQUIVALENT_PROPERTY => writeln!(property_hierarchy, "{}\t{}\t{support}", quad.subject, quad.object)?,
            OWL_SAME_AS => {
                let left = quad.subject.to_string();
                let right = quad.object.to_string();
                let representative = if left <= right { left.clone() } else { right.clone() };
                writeln!(sameas, "{left}\t{representative}\t{support}")?;
                writeln!(sameas, "{right}\t{representative}\t{support}")?;
            }
            _ => writeln!(property, "{}\t{}\t{}\t{support}", quad.predicate, quad.subject, quad.object)?,
        }
        triples.push(triple);
        graphs.push(closure_graph.to_owned());
        supports.push(support);
        kinds.push(kind.to_owned());
        if triples.len() >= row_group_rows { flush_parquet(&mut parquet, &schema, &mut triples, &mut graphs, &mut supports, &mut kinds)?; }
    }
    flush_parquet(&mut parquet, &schema, &mut triples, &mut graphs, &mut supports, &mut kinds)?;
    parquet.close().map_err(|error| OfflineReasoningError::Parquet(error.to_string()))?;
    for writer in [&mut nq, &mut proof, &mut class, &mut property, &mut class_hierarchy, &mut property_hierarchy, &mut sameas] {
        writer.flush()?;
        writer.get_ref().sync_all()?;
    }
    drop((proof, class, property, class_hierarchy, property_hierarchy, sameas));
    for (unsorted, sorted) in [
        (&proof_unsorted_path, &proof_path), (&class_unsorted_path, &class_path),
        (&property_unsorted_path, &property_path), (&class_hierarchy_unsorted_path, &class_hierarchy_path),
        (&property_hierarchy_unsorted_path, &property_hierarchy_path), (&sameas_unsorted_path, &sameas_path),
    ] {
    ...
    for (name, rows) in [
        ("closure.nt", line_count(canonical)?), ("closure.nq", line_count(&nq_path)?),
        ("proof-support.tsv", line_count(&proof_path)?), ("class-extents.tsv", line_count(&class_path)?),
        ("property-extents.tsv", line_count(&property_path)?), ("class-hierarchy.tsv", line_count(&class_hierarchy_path)?),
        ("property-hierarchy.tsv", line_count(&property_hierarchy_path)?), ("sameas-membership.tsv", line_count(&sameas_path)?),
        ("closure.parquet", line_count(canonical)?),
    ] {
        let path = output_root.join(name);
        artifacts.push(OfflineArtifact { relative_path: name.to_owned(), sha256: sha256_path(&path)?, bytes: fs::metadata(&path)?.len(), row_count: rows });
    }
    Ok(artifacts)
}

fn flush_parquet(
    writer: &mut ArrowWriter<File>, schema: &Arc<Schema>, triples: &mut Vec<String>, graphs: &mut Vec<String>,
    supports: &mut Vec<String>, kinds: &mut Vec<String>,
) -> Result<(), OfflineReasoningError> {
    if triples.is_empty() { return Ok(()); }
    let columns: Vec<ArrayRef> = vec![
        Arc::new(StringArray::from(std::mem::take(triples))), Arc::new(StringArray::from(std::mem::take(graphs))),
        Arc::new(StringArray::from(std::mem::take(supports))), Arc::new(StringArray::from(std::mem::take(kinds))),
    ];
    let batch = RecordBatch::try_new(Arc::clone(schema), columns).map_err(|error| OfflineReasoningError::Parquet(error.to_string()))?;
    writer.write(&batch).map_err(|error| OfflineReasoningError::Parquet(error.to_string()))?;
    Ok(())
}

fn semantic_kind(predicate: &str) -> &'static str {
    match predicate {
        RDF_TYPE => "class-extent", RDFS_SUBCLASS | OWL_EQUIVALENT_CLASS => "class-hierarchy",
        RDFS_SUBPROPERTY | OWL_EQUIVALENT_PROPERTY => "property-hierarchy", OWL_SAME_AS => "equality",
        _ => "property-extent",
    }
}
...
fn support_id(root: &str, partition: u32, triple: &str) -> String {
    ...
    hash.update(b"ngkg-exact-support-v1\0"); hash.update(root.as_bytes()); hash.update(b"\0");
    hash.update(partition.to_be_bytes()); hash.update(b"\0"); hash.update(triple.as_bytes());
    format!("urn:ngkg:support:sha256:{}", hex::encode(hash.finalize()))
}

fn merge_equality_membership(inputs: &[PathBuf], output: &Path) -> Result<u64, OfflineReasoningError> {
    let parent = output.parent().ok_or_else(|| OfflineReasoningError::Contract("equality output has no parent".to_owned()))?;
    ...
    for row in BufReader::new(File::open(&sorted)?).lines() {
        ...
        let member = columns.next().ok_or_else(|| OfflineReasoningError::Contract("invalid equality row".to_owned()))?;
        let candidate = columns.next().ok_or_else(|| OfflineReasoningError::Contract("invalid equality row".to_owned()))?;
        if current_member.as_deref() != Some(member) {
            ...
            representative = candidate.to_owned();
...
fn external_sort_unique(input: &Path, output: &Path, rows_in_memory: usize) -> Result<(), OfflineReasoningError> {
    let parent = output.parent().ok_or_else(|| OfflineReasoningError::Contract("sort output has no parent".to_owned()))?;
    ...
    for row in BufReader::new(File::open(input)?).lines() {
        buffer.push(row?);
        if buffer.len() >= rows_in_memory { flush_sort_run(&scratch, &mut buffer, &mut runs)?; }
    }
...
fn flush_sort_run(root: &Path, buffer: &mut Vec<String>, runs: &mut Vec<PathBuf>) -> Result<(), OfflineReasoningError> {
    if buffer.is_empty() { return Ok(()); }
    buffer.sort(); buffer.dedup();
    let path = root.join(format!("sort-{:08}.run", runs.len()));
    let mut writer = BufWriter::new(create_new(&path)?);
    for row in buffer.iter() { writeln!(writer, "{row}")?; }
    writer.flush()?; writer.get_ref().sync_all()?;
    buffer.clear(); runs.push(path); Ok(())
}
...
fn line_count(path: &Path) -> Result<u64, OfflineReasoningError> {
    let mut count = 0_u64;
    for row in BufReader::new(File::open(path)?).lines() { row?; count = checked_add(count, 1, "line")?; }
    Ok(count)
}

fn checked_add(value: u64, increment: u64, label: &str) -> Result<u64, OfflineReasoningError> {
    value.checked_add(increment).ok_or_else(|| OfflineReasoningError::Contract(format!("{label} count overflow")))
}
...
fn validate_sha256(value: &str) -> Result<(), OfflineReasoningError> {
    if value.len() != 64 || !value.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte)) {
        return Err(OfflineReasoningError::Contract("invalid SHA-256".to_owned()));
    }
    Ok(())
}
...
fn verify_file(path: &Path, expected: &str) -> Result<(), OfflineReasoningError> {
    ...
    if sha256_path(path)? != expected { return Err(OfflineReasoningError::Contract(format!("checksum mismatch for {}", path.display()))); }
    Ok(())
}

fn read_json<T: DeserializeOwned>(path: &Path) -> Result<T, OfflineReasoningError> { Ok(serde_json::from_slice(&fs::read(path)?)?) }
fn write_json_new<T: Serialize>(path: &Path, value: &T) -> Result<(), OfflineReasoningError> {
    let mut bytes = serde_json::to_vec_pretty(value)?; bytes.push(b'\n');
    let mut file = create_new(path)?; file.write_all(&bytes)?; file.sync_all()?; Ok(())
}
fn create_new(path: &Path) -> Result<File, std::io::Error> { OpenOptions::new().create_new(true).write(true).open(path) }
fn create_new_root(path: &Path) -> Result<(), OfflineReasoningError> {
    if path.exists() { return Err(OfflineReasoningError::Contract(format!("output root already exists: {}", path.display()))); }
    fs::create_dir_all(path)?; Ok(())
}
fn sync_directory(path: &Path) -> Result<(), OfflineReasoningError> { File::open(path)?.sync_all()?; Ok(()) }
...
mod tests {
    ...
    fn qualified() -> OntologyQualificationRoot {
        OntologyQualificationRoot {
            format_version: 1, tenant_id: Uuid::nil(), dataset_id: Uuid::nil(), operation_id: Uuid::nil(), snapshot_id: Uuid::nil(),
            semantic_compilation_root_sha256: "1".repeat(64), qualification_request_sha256: "2".repeat(64),
            assembly_manifest_sha256: "3".repeat(64), authorized_graph_set_sha256: "4".repeat(64), datatype_policy_sha256: "5".repeat(64),
            synthetic_snapshot_ontology_sha256: "6".repeat(64), owl_signature_sha256: "7".repeat(64),
            owl_profile_qualification_sha256: "8".repeat(64), owl_consistency_qualification_sha256: "9".repeat(64),
            reasoner_report_sha256: "a".repeat(64), finite_closure_sha256: "b".repeat(64), finite_closure_axiom_count: 2,
            reasoner_name: "HermiT".to_owned(), reasoner_version: "1.4.5.519".to_owned(), profile_valid: true,
            consistency_checked: true, consistent: true, qualification_state: "owl2-dl-qualified".to_owned(), publication_state: "inactive".to_owned(),
        }
    }

    #[test]
    fn topology_does_not_change_logical_partitioning() {
        assert_eq!(partition_for("<urn:a> <urn:p> <urn:b> .", 64), partition_for("<urn:a> <urn:p> <urn:b> .", 64));
    }

    #[test]
    fn rejects_unqualified_or_active_roots() {
        let mut root = qualified(); root.publication_state = "active".to_owned();
    ...
    fn exact_closure_round_trip_is_complete_and_inactive() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let plan_path = plan_exact_consequences(&root, &root_sha256, &closure, &plan_root, PlanLimits {
            logical_partitions: 4, max_consequences: 10, rows_in_memory: 2, max_run_bytes: 4096,
        })?;
        ...
        for partition in 0..4 {
            ...
            manifests.push(reduce_exact_partition(
                &plan_path, &plan_sha256, &root, &root_sha256, &plan_root, partition, &output, 2,
            )?);
        }
        let final_path = finalize_offline_reasoning(
            &plan_path, &plan_sha256, &root, &root_sha256, &manifests, &base.join("final"),
        )?;
