...
/// A verified, fixed-width index copied into an anonymous read-only mapping.
/// The staged file is never memory mapped directly.
...
impl VerifiedLocatorIndex {
    pub fn from_staged_file(
        ...
        #[cfg(unix)]
        if metadata.uid() != limits.expected_owner_uid {
            return Err(SliceError::Integrity("staged index owner"));
        }
        let bytes = fs::read(path)?;
        if crate::sha256(&bytes) != expected_index_sha256 {
            return Err(SliceError::Checksum);
        }
        Self::from_verified_bytes(&bytes, limits)
    }

    pub fn from_verified_bytes(bytes: &[u8], limits: IndexLimits) -> Result<Self, SliceError> {
        if bytes.len() < HEADER_BYTES || bytes.len() > limits.maximum_mapped_bytes {
            return Err(SliceError::Limit);
        }
        if &bytes[0..8] != MAGIC
        ...
        let mut anonymous = MmapMut::map_anon(bytes.len())?;
        anonymous.copy_from_slice(bytes);
        let mapping = anonymous.make_read_only()?;
        Ok(Self {
            mapping,
            count,
            total_bytes,
...
mod tests {
    ...
    fn rejects_mutated_index() {
        ...
        assert!(
            VerifiedLocatorIndex::from_verified_bytes(
                &encoded,
                IndexLimits {
                    maximum_records: 10,
