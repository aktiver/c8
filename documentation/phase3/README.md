...
- Rust and Maven builder images whose complete Cargo registry and Maven repository are already populated. Dockerfile execution is network-disabled and both package managers run offline.
...
`phase3/config/images.json` is the authoritative twelve-image inventory. Eleven NGKG images are built; the approved upstream vLLM image is copied by digest into the controlled registry so the exact deployed artifact can be scanned, attested and signed under the same release policy.
...
`verify_and_issue.py` re-verifies every image signature, SBOM attestation and provenance attestation; verifies signed PostgreSQL and provider evidence blobs; recomputes artifact checksums; requires exactly twelve images, four providers and twelve scenarios per provider; and only then emits a checksum-bound certificate.
