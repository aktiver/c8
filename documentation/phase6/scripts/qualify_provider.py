...
class ClusterQualification:
    def __init__(self, config_path: Path, output: Path) -> None:
        ...
        self.recorder = EvidenceRecorder(self.output / "scenarios", self.subject)
    ...
    def inventory(self) -> dict[str, Any]:
        ...
        for node in nodes:
            ...
            zone = labels.get("topology.kubernetes.io/zone") or labels.get("kubernetes.io/hostname")
            zones.add(zone)
    ...
    def verify_autoscalers(self) -> dict[str, Any]:
        ...
        for name in names:
            ...
            observed.append({"name": name, "cpuPercent": 80, "memoryPercent": 80, "maximumReplicas": hpa["spec"]["maxReplicas"]})
    ...
    def qualify_capacity(self) -> dict[str, Any]:
        ...
        observed_points: set[tuple[int, int]] = set()
        ...
        require(required_points <= observed_points, "capacity matrix is incomplete")
        ...
        return {"resources": resources, "trialCount": len(trials), "capacityPoints": len(observed_points), "autoscalingEventCount": len(events), "saturation": response["saturation"], "rawEvidenceSha256": response["evidenceSha256"]}
    ...
    def qualify_provider_integrations(self) -> dict[str, Any]:
        ...
        return {key: response[key] for key in ("workloadIdentity", "longLivedCloudCredentials", "trigIngestion", "artifactRoundTrip", "nodeScaleFromZero", "nodeScaleDown", "highAvailability", "evidenceSha256")}

    def execute(self) -> None:
        started = epoch_ms()
        inventory = self.inventory()
        autoscaling = self.verify_autoscalers()
        self.recorder.record("cluster_inventory", inventory, started, epoch_ms())
        step = epoch_ms(); capacity = self.qualify_capacity(); self.recorder.record("capacity_saturation", capacity, step, epoch_ms())
        step = epoch_ms(); chaos = self.qualify_chaos(); self.recorder.record("chaos_recovery", chaos, step, epoch_ms())
        step = epoch_ms(); integrations = self.qualify_provider_integrations(); self.recorder.record("provider_integration", integrations, step, epoch_ms())
        evidence = {
            "formatVersion": 1,
        ...
        atomic_json(self.output / f"{self.provider}-evidence.json", evidence)
