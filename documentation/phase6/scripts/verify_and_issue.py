...
def validate_prerequisite(path: Path, required: set[str]) -> dict[str, Any]:
...
def validate_supply(path: Path, subject: str) -> dict[str, Any]:
    ...
    require(document.get("imageCount") == 12 and document.get("signed") is True, "twelve signed images were not proven")
...
def main() -> int:
    ...
    for name, scenarios in PREREQUISITES.items():
        ...
        validate_prerequisite(path, scenarios)
        records.append({"id": name, "sha256": sha256_file(path)})
    differential_path = root / "differential-evidence.json"
    validate_differential(differential_path, subject)
    records.append({"id": "native-oracle-differential", "sha256": sha256_file(differential_path)})
    ...
    for provider in sorted(PROVIDERS):
        ...
        records.append({"provider": provider, "id": "provider-capacity-chaos", "sha256": sha256_file(path)})
    supply_path = root / "supply-chain-evidence.json"
    ...
    records.append({"id": "oci-signatures-sboms", "sha256": sha256_file(supply_path)})
    reproducible_path = root / "reproducible-build-evidence.json"
    reproducible = validate_reproducible(reproducible_path, subject)
    records.append({"id": "reproducible-build", "sha256": sha256_file(reproducible_path)})
    defects_path = root / "defect-ledger.json"
    ...
    records.append({"id": "defect-ledger", "sha256": sha256_file(defects_path)})
    statement = {
        "formatVersion": 1,
