...
1. Every SELECT, ASK, CONSTRUCT and DESCRIBE case has an identical semantic result on the native and qualification-oracle lanes. SELECT comparison preserves duplicates and unbound variables. RDF graph results use a checksum-pinned RDFC-1.0 canonicalizer whenever blank nodes exist.
...
7. The same twelve digest-pinned images must run on RKE2, EKS, AKS and GKE. S3, Azure Blob and GCS/RKE2 object-store paths, workload identity, HA and node provisioning are verified through provider drivers.
...
/etc/ngkg-phase6/
  ...
  defect-ledger.json
  providers/{rke2,eks,aks,gke}.json
...
The GitHub workflow provides the same boundary with a protected `ngkg-phase6-release` environment and four independent provider runners.
