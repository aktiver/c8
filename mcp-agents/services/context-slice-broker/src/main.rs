...
async fn main() -> Result<()> {
    ...
    .await?;
    let resource_budget = ResourceBudget::from_cgroup(50, 1)?;
    let cgroup_threads = resource_budget.threads.max(1);
    ...
    let configured_in_flight = positive_usize("NGKG_CONTEXT_MAX_IN_FLIGHT_READS", 64)?;
    let maximum_index_bytes = positive_usize("NGKG_CONTEXT_MAX_INDEX_BYTES", 268_435_456)?;
    let maximum_range_bytes = positive_usize("NGKG_CONTEXT_MAX_RANGE_BYTES", 67_108_864)?;
    let maximum_chunk_bytes = positive_usize("NGKG_CONTEXT_MAX_CHUNK_BYTES", 67_108_864)?;
    let worst_case_read_bytes = maximum_index_bytes
        .checked_add(maximum_range_bytes)
        .and_then(|value| value.checked_add(maximum_chunk_bytes))
        .ok_or_else(|| anyhow::anyhow!("context read memory budget overflow"))?;
    let memory_derived_in_flight = usize::try_from(
        resource_budget.memory_bytes
            / u64::try_from(worst_case_read_bytes).context("context read budget width")?,
    )
    .unwrap_or(usize::MAX)
    .max(1);
    let maximum_in_flight = configured_in_flight.min(memory_derived_in_flight);
    let state = AppState {
        repository,
        store,
        signer,
...
async fn read_range_inner(
    ...
    let bytes = state
        ...
        .await?;
    let stage_dir = state.index_stage.clone();
    let index_sha_for_task = index_sha.clone();
    ...
    // Filesystem writes, fsync, full-file checksum verification, and mmap setup
    // run on Tokio's blocking pool. The final cache name is content-addressed;
    // hard_link publishes without overwriting a concurrently completed entry.
    let index = tokio::task::spawn_blocking(move || -> std::result::Result<VerifiedLocatorIndex, SliceError> {
        let cached = stage_dir.join(format!("{index_sha_for_task}.idx"));
        if !cached.exists() {
            let temporary = stage_dir.join(format!(".{index_sha_for_task}.{}.tmp", Uuid::new_v4()));
            ...
            match std::fs::hard_link(&temporary, &cached) {
                Ok(()) => {
                    let directory = OpenOptions::new().read(true).open(&stage_dir)?;
                    directory.sync_all()?;
                }
        ...
        VerifiedLocatorIndex::from_staged_file(
            &cached,
            &index_sha_for_task,
            index_len,
            limits,
